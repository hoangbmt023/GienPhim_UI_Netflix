...
import { MessageSquare, Clock, CheckCircle, XCircle, AlertCircle, Send, X, Edit2, Bold, Italic, Type, Link, List, Quote, Eye, HelpCircle, Filter } from 'lucide-react';
import ConfirmModal from '@/components/ConfirmModal/ConfirmModal';
import StatusModal from '@/components/StatusModal/StatusModal';

const STATUS_CONFIG = {
  OPEN: { color: 'var(--status-blue, #3b82f6)', bg: 'var(--status-blue-bg, rgba(59, 130, 246, 0.1))', icon: AlertCircle },
  IN_PROGRESS: { color: 'var(--status-yellow, #f59e0b)', bg: 'var(--status-yellow-bg, rgba(245, 158, 11, 0.1))', icon: Clock },
  REPLIED: { color: 'var(--status-green, #10b981)', bg: 'var(--status-green-bg, rgba(16, 185, 129, 0.1))', icon: CheckCircle },
  CLOSED: { color: 'var(--status-gray, #6b7280)', bg: 'var(--status-gray-bg, rgba(107, 114, 128, 0.1))', icon: XCircle },
};
...
export default function ContactPanel() {
  ...
  const renderMarkdown = (text) => {
    if (!text) return <span style={{color: 'var(--text-secondary)'}}>{t.tickets.noContent || 'Không có nội dung...'}</span>;
    let html = text
      .replace(/</g, '&lt;').replace(/>/g, '&gt;')
  ...
  return (
    <div className="admin-panel-content">
      <div className="mod-header-section" style={{ padding: '0 0 20px 0' }}>
